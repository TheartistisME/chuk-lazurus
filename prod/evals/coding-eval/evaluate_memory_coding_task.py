...
PI_MEMORY_PROVIDER = "lazarus-memory"
PI_MEMORY_MODEL = "lazarus-memory-gemma4"
PI_TELEMETRY_NOTE = (
    "Pi runner does not synthesize memory_used telemetry. Use a model endpoint "
    "or wrapper that emits MEMORY_EVAL_TELEMETRY when --require-telemetry is set."
)
...
def prepare_task_dir(root: Path, fixture: CodingFixture, condition: Condition) -> dict[str, Path]:
    ...
    agent_prompt_file = task_dir / "agent_prompt.md"
...
def write_agent_prompt(
    ...
    *,
    ...
    prompt = build_prompt(fixture, condition)
    write_text(
        path,
        normalize_block(
            f"""
            You are running inside an automated coding eval fixture.

            Working directory: the current directory.
            Target file: solution.py
            Visible tests: visible_tests.py
            Hidden tests: hidden_tests.py is withheld from the model-facing prompt.

            Edit solution.py to implement the requested function. Do not edit
            visible_tests.py, hidden_tests.py, task_prompt.md, or this prompt
            file. You may run the visible tests while working. Leave the final
...
def format_agent_command(
    ...
    *,
    ...
    values = {
        ...
        "agent_prompt_file": shlex.quote(str(agent_prompt_file)),
...
def command_with_optional(flag: str, value: str | None) -> list[str]:
...
def first_existing_pi_bin() -> str:
    local_bin = Path.home() / ".pi" / "agent" / "node_modules" / ".bin" / "pi"
    if local_bin.exists():
...
def wait_for_http(url: str, timeout_s: int) -> None:
    ...
    while time.time() < deadline:
        try:
            with urlopen(url, timeout=2) as response:  # noqa: S310 - local eval bridge only.
...
def read_telemetry_file(path: Path) -> dict[str, Any]:
    if not path.exists():
        return {}
    merged: dict[str, Any] = {}
...
def write_pi_memory_models_config(agent_dir: Path, base_url: str) -> None:
                ...
                "providers": {
                    PI_MEMORY_PROVIDER: {
                        ...
                        "models": [
                            {
                                "id": PI_MEMORY_MODEL,
                                "name": "Lazarus MemoryChat Gemma-4",
                                "reasoning": False,
                                "input": ["text"],
...
def start_pi_memory_bridge(
    ...
    *,
    ...
    log_path = task_dir / "pi_memory_bridge.log"
    log_handle = log_path.open("w", encoding="utf-8")
    command = [
        sys.executable,
        str(args.memory_bridge_script),
        "--host",
        "127.0.0.1",
        "--port",
        str(port),
        ...
        "plug_and_play",
        "--memory-mode",
        args.current_memory_mode,
        "--device",
        args.memory_device,
        "--generation-engine",
        args.memory_generation_engine,
        "--target-file",
        target_file.name,
        "--function-name",
    ...
    if args.memory_model_path:
        command.extend(["--model-path", args.memory_model_path])
    ...
    process._memory_eval_log_handle = log_handle  # type: ignore[attr-defined]
    try:
        wait_for_http(f"http://127.0.0.1:{port}/health", args.memory_bridge_start_timeout_s)
    except Exception:
        process.terminate()
        try:
...
def stop_pi_memory_bridge(process: subprocess.Popen[Any] | None) -> None:
    if process is None:
        return
    process.terminate()
    try:
        ...
        process.kill()
        process.wait(timeout=10)
    log_handle = getattr(process, "_memory_eval_log_handle", None)
...
def run_pi_agent(
    ...
    *,
    ...
    pi_bin = args.pi_bin or first_existing_pi_bin()
    command = (
        split_extra_args(pi_bin)
        + command_with_optional("--provider", args.pi_provider)
        + command_with_optional("--model", args.pi_model)
        + split_extra_args(args.pi_extra_args)
        + ["-p", prompt_text]
    )
    if args.pi_openai_base_url:
        env["OPENAI_BASE_URL"] = args.pi_openai_base_url
    if args.pi_openai_api_key:
        env["OPENAI_API_KEY"] = args.pi_openai_api_key
    pi_local_bin = Path.home() / ".pi" / "agent" / "node_modules" / ".bin"
    if pi_local_bin.exists():
        env["PATH"] = f"{pi_local_bin}{os.pathsep}{env.get('PATH', '')}"
...
def run_agent(
    ...
    elif args.agent in {"pi", "pi-memory"}:
        bridge_process: subprocess.Popen[Any] | None = None
        if args.agent == "pi-memory":
            telemetry_file = task_dir / "memory_eval_telemetry.jsonl"
            args.current_memory_mode = condition.memory_mode
            bridge_process, base_url, bridge_log = start_pi_memory_bridge(
                args,
                task_dir=task_dir,
                store_dir=store_dir,
            ...
            pi_agent_dir = task_dir / ".pi-agent"
            write_pi_memory_models_config(pi_agent_dir, base_url)
            env["PI_CODING_AGENT_DIR"] = str(pi_agent_dir)
            env["MEMORY_EVAL_TELEMETRY_FILE"] = str(telemetry_file)
            env["MEMORY_EVAL_BRIDGE_LOG"] = str(bridge_log)
            original_provider = args.pi_provider
            original_model = args.pi_model
            args.pi_provider = PI_MEMORY_PROVIDER
            args.pi_model = PI_MEMORY_MODEL
        try:
            completed = run_pi_agent(
                args,
                cwd=task_dir,
                env=env,
            ...
            if args.agent == "pi-memory":
                args.pi_provider = original_provider
                args.pi_model = original_model
                stop_pi_memory_bridge(bridge_process)
    else:
...
def run_fixture_condition(
    ...
    telemetry.update(read_telemetry_file(paths["task_dir"] / "memory_eval_telemetry.jsonl"))
    ...
    if memory_used is None:
        ...
        if args.agent == "pi":
            notes.append(PI_TELEMETRY_NOTE)
    else:
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        ...
        epilog=textwrap.dedent(
            ...
            Agent command placeholders:
              {task_dir} {target_file} {prompt_file} {agent_prompt_file} {store_dir}
              {memory_mode} {memory_profile} {condition}
              {fixture_id} {function_name} {python}
    ...
    parser.add_argument(
        "--agent",
        choices=("mock", "command", "pi", "pi-memory"),
        default=None,
        help=(
            "Agent runner. Defaults to command when --agent-command is supplied, "
            "otherwise mock."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--pi-bin",
        default=os.environ.get("MEMORY_EVAL_PI_BIN", first_existing_pi_bin()),
        help=(
            "Pi CLI command prefix for --agent pi (default: pi). Can also be "
            "'npx -y @mariozechner/pi-coding-agent'."
        ),
    )
    parser.add_argument(
        "--pi-provider",
        default=os.environ.get("MEMORY_EVAL_PI_PROVIDER"),
        help="Provider passed to Pi, for example openai, gemini, or portkey.",
    )
    parser.add_argument(
        "--pi-model",
        default=os.environ.get("MEMORY_EVAL_PI_MODEL"),
        help="Model passed to Pi, for example google/gemma-4-E2B-it.",
    )
    parser.add_argument(
        "--pi-extra-args",
        default=os.environ.get("MEMORY_EVAL_PI_EXTRA_ARGS"),
        help="Additional Pi CLI args, shell-split before -p.",
    )
    parser.add_argument(
        "--pi-openai-base-url",
        default=os.environ.get("MEMORY_EVAL_PI_OPENAI_BASE_URL"),
        help="Set OPENAI_BASE_URL for Pi, useful with Lazarus OpenAI-compatible server.",
    )
    parser.add_argument(
        "--pi-openai-api-key",
        default=os.environ.get("MEMORY_EVAL_PI_OPENAI_API_KEY"),
        help="Set OPENAI_API_KEY for Pi. Local Lazarus deployments can use a dummy key if allowed.",
    )
    parser.add_argument(
        ...
        / "evals"
        / "coding-eval"
        / "pi_interactive_memory_bridge.py",
        help="Bridge script used by --agent pi-memory.",
    )
    parser.add_argument(
        "--memory-model-path",
        default=os.environ.get("LAZARUS_MODEL"),
        help="Model path/id passed to MemoryChat bridge.",
    )
    parser.add_argument(
        "--memory-device",
        default=os.environ.get("LAZARUS_DEVICE", "cuda"),
        help="Device passed to MemoryChat bridge.",
    )
    parser.add_argument(
        "--memory-generation-engine",
        default=os.environ.get("LAZARUS_GENERATION_ENGINE", "standard"),
        help="Generation engine passed to MemoryChat bridge.",
    )
    parser.add_argument(
        "--memory-max-new-tokens",
        type=int,
        default=int(os.environ.get("LAZARUS_MAX_NEW_TOKENS", "800")),
        help="Max new tokens for each MemoryChat bridge turn.",
    )
    parser.add_argument(
        "--memory-bridge-start-timeout-s",
        type=int,
        default=900,
        help="Seconds to wait for MemoryChat bridge startup/model load.",
    )
    parser.add_argument(
        "--output-root",
        type=Path,
    ...
    if args.agent is None:
        args.agent = "command" if args.agent_command is not None else "mock"
    if args.agent == "command" and not args.agent_command:
        parser.error("--agent command requires --agent-command")
    if args.agent != "command" and args.agent_command:
        parser.error("--agent-command is only valid with --agent command")
    return args
...
def main(argv: list[str] | None = None) -> int:
    ...
    agent_label = args.agent if args.agent != "command" else f"command:{args.agent_command}"
    print("[memory-coding-eval] agent=" + agent_label)
