#!/usr/bin/env python3
"""Build a clause-aligned AUS3000 torch checkpoint variant.

This tool avoids the flat-stream 512-token chunking used by the generic
knowledge builder. Each clause becomes its own retrieval unit when it fits the
window budget; longer clauses are split only within that clause with repeated
metadata on every subchunk.
"""
...
DEFAULT_CHECKPOINT = DEFAULT_INPUT_DIR / "gemma4_aus3000_clause_aligned_variant"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Build a clause-aligned AUS3000 torch checkpoint variant."
    )
    parser.add_argument(
        "--input-dir",
        type=Path,
        default=DEFAULT_INPUT_DIR,
        help="Directory containing AUS3000 clause JSON files",
    )
    parser.add_argument(
        "--model",
        type=Path,
...
def build_prefill_metadata(
    *,
    ...
    metadata = {
        ...
        "source": {
            "name": "AS/NZS 3000 Clause Aligned",
