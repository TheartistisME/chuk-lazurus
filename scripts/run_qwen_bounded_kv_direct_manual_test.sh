...
show_help() {
  cat <<'EOF'
Run the local Qwen manual bounded KV-direct test against chuk-lazurus.
...
Examples:
  scripts/run_qwen_bounded_kv_direct_manual_test.sh
  REPEAT_DOC_COUNT=200 scripts/run_qwen_bounded_kv_direct_manual_test.sh
  HOT_BUDGET_MIB=1024 MODEL_PATH=/models/my-qwen DEVICE=cuda:1 scripts/run_qwen_bounded_kv_direct_manual_test.sh
EOF
}
...
OUT_DIR="${OUT_DIR:-$ROOT_DIR/artifacts/qwen_kv_direct_bounded}"
HOT_BUDGET_MIB="${HOT_BUDGET_MIB:-1024}"
...
if [[ ! "$HOT_BUDGET_MIB" =~ ^[0-9]+$ ]] || (( HOT_BUDGET_MIB < 1 )); then
  echo "ERROR: HOT_BUDGET_MIB must be an integer >= 1." >&2
  exit 1
fi

echo "Running Qwen bounded KV-direct manual test..."
...
echo "  budget  : ${HOT_BUDGET_MIB} MiB"
echo
...
def resolve_forward_kwargs(model) -> dict[str, object]:
...
def extract_logits_and_cache(outputs) -> tuple[torch.Tensor, object]:
    ...
    if isinstance(outputs, tuple) and outputs:
        ...
        return logits, cache
    raise RuntimeError("Bounded KV-direct expected model outputs with logits and past_key_values.")
...
def build_text_only_position_ids(model, batch_size: int, start: int, length: int, device) -> torch.Tensor:
    ...
    if "Qwen3_5MoeForConditionalGeneration" in model_name:
        return torch.stack([text_positions, text_positions, text_positions, text_positions], dim=0)
...
def cache_seq_length(past_key_values) -> int:
    ...
    if hasattr(past_key_values, "get_seq_length"):
        ...
        return int(seq_len.item()) if isinstance(seq_len, torch.Tensor) else int(seq_len)
...
def cache_memory_breakdown(past_key_values) -> dict[str, int]:
    seq_length = cache_seq_length(past_key_values)
    variable_bytes = 0
    fixed_bytes = 0
    kv_layers = 0
    ...
    if hasattr(past_key_values, "layers"):
        for layer in past_key_values.layers:
            ...
            if isinstance(keys, torch.Tensor) and keys.numel() > 0:
                layer_variable += tensor_nbytes(keys)
            if isinstance(values, torch.Tensor) and values.numel() > 0:
                layer_variable += tensor_nbytes(values)
            if layer_variable > 0:
            ...
            for attr in ("conv_states", "recurrent_states"):
                layer_fixed += tensor_nbytes(getattr(layer, attr, None))
            if layer_fixed > 0:
                fixed_bytes += layer_fixed
                linear_state_layers += 1
    elif isinstance(past_key_values, (list, tuple)):
        for layer in past_key_values:
            if not isinstance(layer, (list, tuple)) or len(layer) < 2:
                continue
            variable_bytes += tensor_nbytes(layer[0])
            variable_bytes += tensor_nbytes(layer[1])
            kv_layers += 1
    ...
    return {
        ...
        "kv_layers": kv_layers,
        "linear_state_layers": linear_state_layers,
    }


def max_hot_tokens_for_budget(past_key_values, budget_bytes: int) -> tuple[int, dict[str, int]]:
    stats = cache_memory_breakdown(past_key_values)
    ...
    return max(1, available_variable_bytes // variable_bytes_per_token), stats


def _set_layer_seq_length(layer, keep_last_tokens: int) -> None:
    if hasattr(layer, "cumulative_length"):
        current = getattr(layer, "cumulative_length")
        new_length = min(
            keep_last_tokens,
            int(current.item()) if isinstance(current, torch.Tensor) else int(current),
        )
...
def trim_cache_to_last_tokens(past_key_values, keep_last_tokens: int):
    ...
    if hasattr(past_key_values, "layers"):
        for layer in past_key_values.layers:
            keys = getattr(layer, "keys", None)
            values = getattr(layer, "values", None)
            if isinstance(keys, torch.Tensor) and keys.numel() > 0 and keys.shape[-2] > keep_last_tokens:
                layer.keys = keys[..., -keep_last_tokens:, :].contiguous()
            if isinstance(values, torch.Tensor) and values.numel() > 0 and values.shape[-2] > keep_last_tokens:
                layer.values = values[..., -keep_last_tokens:, :].contiguous()
            _set_layer_seq_length(layer, keep_last_tokens)
        return past_key_values
    ...
    raise RuntimeError(f"Unsupported past_key_values type for bounded trim: {type(past_key_values).__name__}")


def enforce_last_token_budget(past_key_values, keep_last_tokens: int) -> tuple[object, int]:
    current_seq = cache_seq_length(past_key_values)
    if current_seq <= keep_last_tokens:
        return past_key_values, 0
    evicted = current_seq - keep_last_tokens
    return trim_cache_to_last_tokens(past_key_values, keep_last_tokens), evicted
...
hot_budget_mib = int(os.environ["HOT_BUDGET_MIB"])
budget_bytes = hot_budget_mib * 1024 * 1024
...
forward_kwargs = resolve_forward_kwargs(model)
...
try:
    ...
    input_ids = prompt_batch["input_ids"].to(device=device, non_blocking=True)
    ...
    stop_token_ids = set(int(token_id) for token_id in generation_stop_tokens)
    batch_size = int(input_ids.shape[0])
    total_seen_tokens = int(input_ids.shape[1])
    generated_token_ids: list[int] = []
    stop_reason = "max_tokens"
    total_evicted_tokens = 0
    prefill_evicted_tokens = 0
    window_start = 0
    ...
    with torch.inference_mode():
        prefill_outputs = model(
            ...
            position_ids=build_text_only_position_ids(
                model, batch_size, 0, total_seen_tokens, input_ids.device
            ),
            use_cache=True,
            **forward_kwargs,
        )
        logits, past_key_values = extract_logits_and_cache(prefill_outputs)
        if past_key_values is None:
            raise RuntimeError(
                "STRICT: bounded kv_direct expected the model prefill to return past_key_values."
            )

        max_hot_tokens, initial_cache_stats = max_hot_tokens_for_budget(past_key_values, budget_bytes)
        prefill_hot_limit = max(1, max_hot_tokens - max_new_tokens)
        past_key_values, prefill_evicted_tokens = enforce_last_token_budget(
            past_key_values, prefill_hot_limit
        )
        total_evicted_tokens += prefill_evicted_tokens

        bounded_cache_stats = cache_memory_breakdown(past_key_values)
        window_start = total_seen_tokens - bounded_cache_stats["seq_length"]
        max_observed_hot_bytes = bounded_cache_stats["total_bytes"]
        max_observed_budget_pct = (
            bounded_cache_stats["total_bytes"] * 100.0 / budget_bytes if budget_bytes > 0 else 0.0
        )
        ...
        for step in range(max_new_tokens):
            ...
            hot_seq_len = cache_seq_length(past_key_values)
            ...
            decode_outputs = model(
                ...
                position_ids=build_text_only_position_ids(
                    model, batch_size, total_seen_tokens, 1, next_token.device
                ),
                past_key_values=past_key_values,
                use_cache=True,
                **forward_kwargs,
            )
            logits, past_key_values = extract_logits_and_cache(decode_outputs)
            if past_key_values is None:
                raise RuntimeError(
            ...
            total_seen_tokens += 1
            past_key_values, evicted = enforce_last_token_budget(past_key_values, max_hot_tokens)
            total_evicted_tokens += evicted

            bounded_cache_stats = cache_memory_breakdown(past_key_values)
            window_start = total_seen_tokens - bounded_cache_stats["seq_length"]
            if bounded_cache_stats["total_bytes"] > max_observed_hot_bytes:
                max_observed_hot_bytes = bounded_cache_stats["total_bytes"]
            budget_pct = (
                bounded_cache_stats["total_bytes"] * 100.0 / budget_bytes if budget_bytes > 0 else 0.0
            )
            if budget_pct > max_observed_budget_pct:
                max_observed_budget_pct = budget_pct
    ...
    path_value = "torch.kv_direct.bounded_past_key_values"
    ...
    tokens_per_second = output_token_count / generate_elapsed if generate_elapsed > 0 else 0.0
    final_cache_stats = cache_memory_breakdown(past_key_values)
except Exception as exc:
    failure_payload = make_failure_payload(
        stage="generate",
        exc=exc,
...
checks = {
    "path_is_bounded_kv_direct": path_value == "torch.kv_direct.bounded_past_key_values",
    ...
    "bounded_hot_state_within_budget": final_cache_stats["total_bytes"] <= budget_bytes,
}
...
overall_status = (
    "PASS"
    if checks["path_is_bounded_kv_direct"]
    and checks["bounded_hot_state_within_budget"]
    and content_pass
    else "FAIL"
)

print_section("Bounded KV Budget")
print_kv_table(
    [
        ("hot_budget_mib", hot_budget_mib),
        ("budget_bytes", budget_bytes),
        ("initial_prefill_seq", initial_cache_stats["seq_length"]),
        ("initial_prefill_hot_bytes", initial_cache_stats["total_bytes"]),
        ("initial_fixed_bytes", initial_cache_stats["fixed_bytes"]),
        ("initial_var_bytes_per_token", initial_cache_stats["variable_bytes_per_token"]),
        ("initial_kv_layers", initial_cache_stats["kv_layers"]),
        ("initial_linear_state_layers", initial_cache_stats["linear_state_layers"]),
        ("max_hot_tokens", max_hot_tokens),
        ("prefill_hot_limit", prefill_hot_limit),
        ("prefill_evicted_tokens", prefill_evicted_tokens),
    ]
)
...
print_kv_table(
    [
        ...
        ("final_hot_tokens", final_cache_stats["seq_length"]),
        ("final_hot_bytes", final_cache_stats["total_bytes"]),
        ("total_evicted_tokens", total_evicted_tokens),
        ("max_observed_hot_bytes", max_observed_hot_bytes),
        ("max_observed_budget_pct", f"{max_observed_budget_pct:.2f}%"),
    ]
)
...
summary_payload = {
    ...
    "generation": {
        ...
        "final_hot_tokens": final_cache_stats["seq_length"],
        "final_hot_bytes": final_cache_stats["total_bytes"],
        "final_fixed_bytes": final_cache_stats["fixed_bytes"],
        "final_var_bytes_per_token": final_cache_stats["variable_bytes_per_token"],
        "total_evicted_tokens": total_evicted_tokens,
        "max_observed_hot_bytes": max_observed_hot_bytes,
        "max_observed_budget_pct": max_observed_budget_pct,
    },
    "checks": checks,
...
if path_value != "torch.kv_direct.bounded_past_key_values":
    print(
        f"ERROR: expected PATH=torch.kv_direct.bounded_past_key_values, got {path_value!r}",
        file=sys.stderr,
    )
    summary_payload["overall_status"] = "FAIL"
    write_summary(summary_file, summary_payload)
    raise SystemExit(3)
if final_cache_stats["total_bytes"] > budget_bytes:
    print(
        "ERROR: bounded hot cache exceeded the requested budget.",
        file=sys.stderr,
    )
    summary_payload["overall_status"] = "FAIL"
    write_summary(summary_file, summary_payload)
