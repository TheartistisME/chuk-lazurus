...
def _build_virtual_router():
    ...
    class VirtualRouter(nn.Module):
        ...
        def __call__(self, x: mx.array) -> tuple[mx.array, mx.array, dict[int, mx.array]]:
            ...
            self._last_virtual_logits = virtual_logits
...
def _get_virtual_router():
    global _VIRTUAL_ROUTER_CLS
    if _VIRTUAL_ROUTER_CLS is None:
        _VIRTUAL_ROUTER_CLS = _build_virtual_router()
    return _VIRTUAL_ROUTER_CLS


def __getattr__(name: str):
    if name == "VirtualRouter":
        value = _get_virtual_router()
        globals()[name] = value
        return value
