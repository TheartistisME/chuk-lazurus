#!/usr/bin/env python3
"""AUS3000 + Gemma + CUDA strict-injection demo.

Apollo-11-pattern adaptation for the AUS3000 clause-aligned store. Every known
silent-fallback path is turned into a HARD FAILURE so the presence of an answer
cannot be confused with successful residual injection.
...
Usage:
  uv run python examples/inference/demo_c_aus3000_torch_strict.py \\
      --question "What does clause 1.4.72 define?"

Defaults target the cached google/gemma-4-E2B-it model (hidden=1536, 35 layers)
which matches the AUS3000 store manifest (crystal_layer=29, boundary width 1536).
"""
...
DEFAULT_STORE = "/mnt/c/Users/jehma/Desktop/AI/TradeGuru/lewisnjehmal/downloads/standards/embeddings/AS_NZS_3000-2018/as_nzs_3000_2018_store"
...
def main() -> int:
    ...
    print(_bold("=== STRICT AUS3000 TORCH INJECTION DEMO ==="))
    ...
    # Build prompt (Apollo-demo pattern, adapted for AUS3000)
    system = (
        "You answer AS/NZS 3000 electrical-standard questions using only the store context "
        "provided by Lazarus. Be concise; quote clause IDs when known."
    )
    user_content = (
